...
class ClusterAdminController extends CRUDController
{
    ...
    public function showAction($id = null)
    {
        ...
        $serverRepo =  $this->getDoctrine()->getRepository('Oblady\ZuulBundle\Entity\Server');
        $associatedServers  = $serverRepo->findByClusterId($object->getId());
        
        $unassociatedServers =array_diff_key($serverRepo ->findAllIndexed(),$associatedServers) ;
        ...
        return $this->render($this->admin->getTemplate('show'), array(
            ...
            'associated' => $associatedServers,
            'unassociated' => $unassociatedServers,
        ));
    }
    
     public function addServerAction($id = null,$fk = null)
     {
           $answer = array();
           $answer['error'] = 0;
           ...
           try {
                ...
                $remote = $this->getDoctrine()->getRepository('\Oblady\ZuulBundle\Entity\Server')->find($fk);
                if (!$remote) {
                     throw new NotFoundHttpException(sprintf('unable to find the remote object with id : %s', $fk));
                }

                $manager = $this->container->get('oblady_zuul.cluster_manager');
                $manager->setCluster($object);
                $manager->linkServer($remote);
     ...
     public function removeServerAction($id = null,$fk = null)
     {
           $answer = array();
           $answer['error'] = 0;
